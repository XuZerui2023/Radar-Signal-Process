...
function [sig_data_DBF_allprts, servo_angle, frameCompleted, is_global_stream_end] = FrameDataRead_xzr(orgDataFilePath, DBF_coeffs_data_C, Sig_Config, frameRInd)
...
% 信号处理部分
    ...
    % 1. 读取帧头信息
        ...
        frame_no(current_prt + 1) = datahead(1);                  % 帧号，迭代的最后一个prt对应的帧（从0开始），第1-4字节
        pulse_no = mod(datahead(3),2^16);        % PRT号（9-12字节），一个完整帧的PRT是0-331，取余（保留低16位即9-10字节）
        freq_no = datahead(3)/2^16;                               % 频点号，将9-12字节内容向右平移16位（第11-12字节）
        channel_num = mod(datahead(4),2^8);                       % 通道数，取余保留低8位(第13字节)
        current_servo_angle = mod(datahead(5),2^16);     % 伺服方位角，单位0.1°（第17-18字节），这个数组是否是后面所需的角编码？
        pulse_data_num = datahead(7);                             % 一个PRT采样点数（第25-28字节）
        data_type = mod(datahead(8),2^8);                         % 数据类型,0=ADC数据,1=DDC数据;2=DBF数据 （第29字节）
        pulse_num = mod(floor(datahead(8)/2^8),2^16);             % 一帧PRT个数（30-31字节）
        radar_type = mod(floor(datahead(8)/2^24),2^8);            % 雷达型号，0-X3D3K，1-C3D5K，2-X3D8K，3-C3D8K，4-KuP4K（第32字节）
        timer_cnt(current_prt + 1) = datahead(9) + datahead(10)*2^32;   % 时标（第33-36字节作为低32位 + 第37-40字节作为高32位）
        s_pluse_dots = mod(datahead(11),2^16);                    % 短脉冲采样点数（第41-42字节）
        m_pluse_dots = mod(floor(datahead(11)/2^16),2^16);        % 中脉冲采样点数（第43-44字节）
        l_pluse_dots = mod(datahead(12),2^16);                    % 长脉冲采样点数（第45-46字节）

        
        % 验证 pulse_data_num 的有效性
        if pulse_data_num <= 0
            warning('frameDataRead_A_xzr_V2: Invalid pulse_data_num (%d) read from frame head for frame %d, PRT %d. Treating as corrupted PRT.', pulse_data_num, frameRInd, current_prt + 1);
