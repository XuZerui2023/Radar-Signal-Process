...
FrameHead_information{length(frame_range), 1} = zeros; 


% 创建所有输出目录
config.output_paths.framehead = fullfile(base_path, num2str(n_exp), 'Framehead_information');
...
for frame_to_load = frame_range
    ...
    FrameHead_information{current_frame_index, 1} = framehead;             % 存储帧头信息的元胞数组


    % % 保存帧头信息
    % if config.save_options.save_frameheads_mat
    %     FrameHead_information_save = framehead;
...
figure(1)
plot(plot_frame_no, plot_servo_angle2)
xlabel('帧数'); % 如果有实际的距离值，可以使用实际值
ylabel('伺服角度');

north_angle = 307;
fix_angle = 35;
correct_angle = fun_correct_servo_angle(plot_servo_angle,north_angle,fix_angle);
figure(2)
plot(correct_angle)
xlabel('帧数'); % 如果有实际的距离值，可以使用实际值
ylabel('伺服角度');
