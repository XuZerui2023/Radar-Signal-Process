...
% PROCESS_STAGE3_DETECTION - 目标检测阶段 (处理流程第三阶段)
%
% 本函数是信号处理流程重构后的第三阶段。其核心职责是"发现目标"，并为下一阶段的
% "精算参数"准备好所有必要的原始信息。
% process_stage3_detection.m 
% --- 本脚本工作流程 ---
% 1. 加载由第二阶段(MTD)生成的、包含13个原始俯仰波束的RDM数据。
...
%    打包记录下来。
% 6. 将记录好的信息保存为一个"初步检测日志"(.mat)文件，作为第四阶段的输入。
%
% 输入参数:
%   config - (struct) 全局配置结构体，包含了所有路径和处理参数。
%
% 输出:
%   无直接返回值。函数会将处理结果以.mat文件形式保存到磁盘。
%
%  修改记录
%  date       by      version   modify
%  25/07/10   XZR      v1.0      创建，用于替代旧的CFAR处理流程
%  25/07/11   XZR      v1.1      添加详细注释

%% 1. 初始化与参数加载
% =========================================================================
fprintf('--- STAGE 3: 目标检测开始 ---\n');

% --- 从全局配置中获取所需路径 ---
% 输入路径：第二阶段(MTD)处理结果的存放位置
% mtd_data_path = config.output_paths.mtd; 

% 输出路径：为本阶段创建新的文件夹，用于存放初步检测日志
prelim_detection_path = fullfile(config.base_path, num2str(config.n_exp), 'prelim_detection_log');
if ~exist(prelim_detection_path, 'dir'), mkdir(prelim_detection_path); end

% --- 从全局配置中获取所需处理参数 ---
frame_range = config.frame_range;
beam_num = config.Sig_Config.beam_num;
win_size = config.mtd.win_size;
% 从config中获取CFAR参数，这些参数由主控脚本统一配置
CFAR_params = config.cfar; 

%% 2. 主处理循环 (遍历所有数据帧)
% =========================================================================
% 注意：MTD处理阶段是两帧并作一帧输出的，所以循环到倒数第二帧
for frame_idx = frame_range(1:end-1) 
    
    fprintf('  > 正在检测帧 #%d...\n', frame_idx);
    
    % --- 加载当前帧的MTD结果 ---
    mtd_file = fullfile(mtd_data_path, ['frame_', num2str(frame_idx), '.mat']);
    if ~exist(mtd_file, 'file')
        warning('MTD文件 %s 不存在，跳过此帧。', mtd_file);
        continue;
    end
    ...
    for s = 1:win_size
        ...
        for i = 1:(beam_num - 1)
            ...
            % 1. 获取相邻的两个原始RDM (复数矩阵)
            %    squeeze() 用于移除单维度，将 (1 x vel x range) 变为 (vel x range)
            RDM_beam1 = squeeze(MTD_win_all_beams{i}(s, :, :));
            RDM_beam2 = squeeze(MTD_win_all_beams{i+1}(s, :, :));
            
            % 2. 合成"和信号"幅度矩阵，专门用于CFAR检测
            %    根据参考项目经验，使用幅度求和的方式构造和信号，信噪比最高。
            RDM_sum_amp = abs(RDM_beam1) + abs(RDM_beam2);
            
            % 3. 在"和信号"上执行CFAR检测
            %    调用本地封装的CFAR函数，得到0-1标志矩阵。
    ...
    if ~isempty(prelim_log_this_frame)
        output_filename = fullfile(prelim_detection_path, ['prelim_log_frame_', num2str(frame_idx), '.mat']);
        % 将 prelim_log_this_frame 变量保存到 .mat 文件中
        save(output_filename, 'prelim_log_this_frame');
        fprintf('  > 帧 #%d 检测到 %d 个潜在目标，初步日志已保存。\n', frame_idx, length(prelim_log_this_frame));
    else
        fprintf('  > 帧 #%d 未检测到目标。\n', frame_idx);
    end
...
% 建议：将您项目中经过完整测试的CFAR函数逻辑移植到这里，以确保最佳性能。
% 下面的实现是一个基于您项目代码的简化框架。
% ========================================================================
