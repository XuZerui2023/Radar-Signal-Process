...
%   cfar_flag_all_beams   - (cell) 12x1的元胞数组，每个元胞存储一个"和波束"的CFAR标志矩阵。
%
%  修改记录
...
function [prelim_log_this_frame, cfar_flag_all_beams] = process_stage3_detection(mtd_results_one_frame, angle, config, frame_idx)
%% 1. 初始化与参数加载
% =========================================================================
...
% 初始化一个元胞数组，用于存储每一对"和波束"的CFAR检测结果
cfar_flag_all_beams = cell(beam_num - 1, 1);
...
for s = 1 : win_size          % 目前切片窗口为 4
    ...
    for i = 1:(beam_num - 1)  % 目前波束对为 12
        ...
        % --- 【新增】将当前"和波束"的CFAR结果存入总的元胞数组中 ---
        % 我们需要将每个切片的结果叠加起来，形成一个三维矩阵
        if s == 1
            % 如果是第一个切片，直接创建
            cfar_flag_all_beams{i} = cfar_flag_matrix;
        else
            % 如果不是第一个切片，在第三个维度上拼接
            cfar_flag_all_beams{i}(:,:,s) = cfar_flag_matrix;
        end
        % 注意：为了保持与旧格式的兼容性，我们可能需要调整这里的维度顺序。
        % 旧格式是 (slice, vel, range)。让我们调整一下。
        temp_storage_for_slice(s, :, :) = cfar_flag_matrix;

        % 4. 查找所有被检测为目标的位置
        [target_v_indices, target_r_indices] = find(cfar_flag_matrix);
        % if ~isempty(target_v_indices)                                 % 可以增添一个速度筛选选项
        %     physical_velocities_ms = v_axis(target_v_indices);
        %     velocity_filter_mask = (physical_velocities_ms > 2) | (physical_velocities_ms < -2);
        %     target_v_indices = target_v_indices(velocity_filter_mask);
        %     target_r_indices = target_r_indices(velocity_filter_mask);
        % end
