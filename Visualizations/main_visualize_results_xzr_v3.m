...
T_CFAR = 10;        % CFAR门限因子 (必须与生成数据时使用的编号一致)
...
MTD_data_one_beam1 = MTD_win_all_beams{beam_to_plot};
MTD_data_one_beam2 = MTD_win_all_beams{beam_to_plot + 1};
if slice_to_plot > size(MTD_data_one_beam1, 1) || slice_to_plot < 1, error('切片索引无效'); end
mtd_to_plot1 = squeeze(MTD_data_one_beam1(slice_to_plot, :, :));
mtd_to_plot2 = squeeze(MTD_data_one_beam2(slice_to_plot, :, :));

% --- 提取对应的CFAR数据 ---
cfarFlag_one_beam1 = cfarFlag_win_all_beams{beam_to_plot};
cfarFlag_one_beam2 = cfarFlag_win_all_beams{beam_to_plot + 1};

cfar_to_plot1 = squeeze(cfarFlag_one_beam1(slice_to_plot, :, :));
cfar_to_plot2 = squeeze(cfarFlag_one_beam2(slice_to_plot, :, :));

%% 5. 调用绘图函数进行可视化
fprintf('正在为第 %d 帧, 第 %d 波束, 第 %d 切片的数据绘图...\n', frame_to_load, beam_to_plot, slice_to_plot);

% 调用新的仪表盘绘图函数
fun_plot_cfar_dashboard_v3(mtd_to_plot1, mtd_to_plot2, cfar_to_plot1, cfar_to_plot2, params);
