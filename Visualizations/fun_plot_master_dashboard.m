...
switch plot_option
    case 'summary'
        ...
        fig5 = figure(5);
        ax5 = axes(fig5);
        scatter(ax5, all_ranges, all_elevations, 20, all_velocities, 'filled', 'MarkerFaceAlpha', 0.7);
        grid(ax5, 'on'); xlabel(ax5, '距离 (m)'); ylabel(ax5, '俯仰角 (°)');
        title(ax5, '距离-俯仰角图 (RHI) - 颜色代表速度');
        c2 = colorbar(ax5); c2.Label.String = '速度 (m/s)';

        % 子图1: PPI (距离-方位图)
        fig6 = figure(6)
        ax6 = axes(fig6);
        scatter(ax6, all_ranges, corrected_azimuths, 20, all_heights, 'filled', 'MarkerFaceAlpha', 0.7); % polarscatter 期望输入的是弧度
        title(ax6, '距离方位图 - 颜色代表高度');
        %c1 = colorbar(ax1); c1.Label.String = '高度 (m)';
