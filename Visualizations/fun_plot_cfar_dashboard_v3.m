function fun_plot_cfar_dashboard_v3(mtd_signal1, mtd_signal2, cfar_flag1, cfar_flag2, sys_params)
...
[num_vel_bins, num_range_bins] = size(mtd_signal1);

% --- 数据归一化和对数化 ---
mtd_abs1 = abs(mtd_signal1);
mtd_abs2 = abs(mtd_signal2);
max_val = max(mtd_abs1(:));
if max_val == 0, max_val = 1; end
% mtd_log = 20 * log10(mtd_abs / max_val + 1e-9);
mtdsum_log = 20 * log10(mtd_abs1 + mtd_abs2);
mtddiff_log = 20 * log10(mtd_abs1 - mtd_abs2);
...
detected_signals = mtd_abs1 * cfar_flag1;
[peak_signal_val, peak_idx] = max(detected_signals(:));

if peak_signal_val > 0
    % 如果有检测到的目标，则以最强目标为中心进行切片
    [peak_vel_idx, peak_range_idx] = ind2sub(size(mtd_abs1), peak_idx);
else
    % 如果没有检测到目标，则以整个画面的最强点为中心进行切片
    [~, peak_idx] = max(mtd_abs1(:));
    [peak_vel_idx, peak_range_idx] = ind2sub(size(mtd_abs1), peak_idx);
end
...
figure(2);
mesh(r_axis, v_axis, abs(mtddiff_log));
xlabel('距离 (m)');
ylabel('速度 (m/s)');
zlabel('差波束幅度 (dB)');
title('3D 速度-距离图');
