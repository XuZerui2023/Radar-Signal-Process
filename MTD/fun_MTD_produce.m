...
% 只在这里控制是否画图
show_PC = params.debug.show_PC;   % 脉冲压缩结果显示
show_FFT = params.debug.show_FFT; % 速度维显示
graph = params.debug.graph;       % 是否画图 
...
% 2.3 雷达系统参数
prt   = params.prt;             % X波段雷达PRT
prf   = params.prf;
prtNum = params.prtNum;         % 每帧信号的脉冲数
fs = params.fs;                 % 生成原始信号的采样频率，每秒25,000,000次采样
ts = 1/fs;                      % 采样时间间隔
deltaR = params.deltaR;         % 距离分辨率单元

f0 = 0*MHz;                     % 中心频率
fc = params.fc;                 % X波段雷达中心频率
wavelength = params.wavelength; % 信号波长

B = params.B;                   % 带宽
tao1  = params.tao(1);          % 脉冲1脉宽
tao2  = params.tao(2);          % 脉冲2脉宽
tao3  = params.tao(3);          % 脉冲3脉宽
K1    = params.K1;              % 短脉冲调频斜率（简单脉冲不需要）
K2    = params.K2;              % 中脉冲负线性调频斜率
K3    = params.K3;              % 长脉冲线性调频斜率

point_prt  = params.point_prt(1); % 总PRT采集点数
point_prt1 = params.point_prt(2); % 1脉冲区间点数
point_prt2 = params.point_prt(3); % 2脉冲区间点数 
point_prt3 = params.point_prt(4); % 3脉冲区间点数
...
%% 6.杂波抑制
MTD_Signal = fun_0v_pressing(MTD_Signal);   % 0速抑制，压制0速附近的峰值
