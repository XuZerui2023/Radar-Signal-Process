...
%     is_global_stream_end: (logical) 数据流是否结束的标志。
%
% 修改记录:
...
% 信号处理部分
    ...
    while current_prt < prtNum
        % 打印当前处理的逻辑帧和PRT索引，可能会影响程序处理速度，可以注释掉
        % fprintf('FrameDataRead_xzr: 正在处理逻辑帧 %d 的第 %d 个PRT。\n', frameRInd, current_prt + 1);

    % 1. 读取帧头信息
        [datahead_raw_bytes, actual_len_head, is_stream_end_head, current_file_index] = read_continuous_file_stream(bytesFrameHead, orgDataFilePath);
    ...
    % 2. 读取（辅助数据）实时参数
        [aux_data_raw, actual_len_aux, is_stream_end_aux, current_file_index] = read_continuous_file_stream(bytesFrameRealtime, orgDataFilePath);
    ...
    % 4. 读取信号数据 
        [signal_data_raw, actual_len_signal, is_stream_end_signal, current_file_index] = read_continuous_file_stream(pulse_data_size, orgDataFilePath);
    ...
    % 6. 读取帧尾数据
        [~, actual_len_tail, is_stream_end_tail, current_file_index] = read_continuous_file_stream(bytesFrameEnd, orgDataFilePath);
    ...
    % 7. 判断DDC抽取倍数（画频谱图需要）
        ...
        frame_headers(current_prt_index).bin_num = current_file_index; % 把原始文件索引号保存到frame_headers 结构体中 
    end
