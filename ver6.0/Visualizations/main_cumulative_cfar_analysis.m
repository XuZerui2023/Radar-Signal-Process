...
% 25/07/04   XZR      v1.0      该函数用于画累积cfar检测图，将检测点都画到一张图上
% 25/07/05   XZR      V2.0      增加了绘图选项，可以选择生成累积PPI图或累积距离-速度图
% 25/07/06   XZR      V3.0      增加了新的绘图选项 'az_range'，用于生成累积的距离-方位图
%                               
%
% 未来改进：加速处理，利用matlab并行处理加速，将for循环用parfor循环代替
clc; clear; close all;
...
% --- 新功能开关 ---
options.save_detection_log = true;    % 开关，保存检测点的数据
options.save_cumulative_image = true; % 开关，保存累计检测点图像
...
if ~isempty(detection_log)
    switch plot_option
        ...
        case 'summary'
            h_fig = fun_plot_summary_dashboard(detection_log, params, NorthAngle, FixAngle);
