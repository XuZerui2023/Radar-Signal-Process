...
% --- 1.1 流程控制 --- 
config.frame_range = 0 : 50;                       % 指定要处理的帧范围
config.save_options.save_frameheads_mat = false;   % 开关：是否保存读取文件时检测的文件帧头信息
config.save_options.save_iq_mat = false;           % 开关：是否保存第一阶段的.mat格式I/Q数据
config.save_options.save_mtd_mat = false;          % 开关：是否保存第二阶段的MTD结果
config.save_options.save_cfar_mat = false;         % 开关：是否保存第三阶段的CFAR结果
config.save_options.save_beam_sum_cfar_mat = true; % 开关：是否保存第三阶段和波束CFAR目标检测结果
...
[iq_data1, angle1, frame_headers1, success, is_end] = process_stage1_read_data(config.frame_range(1), config, DBF_coeffs_data_C);  % bin_to_mat逻辑封装
...
for frame_idx = 1 : length(config.frame_range) - 1
    ...
    [iq_data2, angle2, frame_headers2, success, is_end] = process_stage1_read_data(next_frame_num, config, DBF_coeffs_data_C);
    ...
    [mtd_results, angles_wins] = process_stage2_mtd(iq_data1, iq_data2, angle1, angle2, config); % 假设您已将MTD逻辑封装
    ...
    % --- STAGE 3: 和波束波束成形及CFAR检测 ---
    
    beamsum_cfar_log = process_stage3_detection(mtd_results, config, current_frame_num);
    if config.save_options.save_beam_sum_cfar_mat % 可以在config中增加新的保存开关
        save(fullfile(config.output_paths.beam_sum_cfar, ['frame_', num2str(current_frame_num), '.mat']), 'beamsum_cfar_log');
