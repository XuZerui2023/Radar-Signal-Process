...
tic;
win_size = 4;  % 窗口大小设置为4
mkdir(['D:\MATLAB_Project\X3D8K DMX回波模拟状态采集数据250520\X3D8K DMX回波模拟状态采集数据250520\X8数据采集250522\1\',num2str(n_exp),'/MTD_data_win',num2str(win_size)]);     % 创建存储MTD数据的文件夹，路径根据实验编号和窗口大小动态生成
fileFolder = fullfile('D:\MATLAB_Project\X3D8K DMX回波模拟状态采集数据250520\X3D8K DMX回波模拟状态采集数据250520\X8数据采集250522\1\',num2str(n_exp),'/BasebandRawData_mat');  % 设置基带原始数据的文件夹路径
...
for frameRInd = 0:15                                 % 循环索引0:3000，表示处理的帧编号
echo_now = load(['D:\MATLAB_Project\X3D8K DMX回波模拟状态采集数据250520\X3D8K DMX回波模拟状态采集数据250520\X8数据采集250522\1\',num2str(n_exp),'/BasebandRawData_mat/frame_',num2str(frameRInd),'.mat']); % 加载当前帧（由frameRInd指定）的.mat文件并赋值给echo_now，该文件包含该帧的基带信号数据。
echo_now_0 = echo_now.echoData_Frame_0;    % 选择波束     % 提取当前帧信号数据
echo_now_1 = echo_now.echoData_Frame_1;    % 选择波束
angles_now = echo_now.angleCodeSeries;     % 选择波束     % 提取当前帧角度编码数据
