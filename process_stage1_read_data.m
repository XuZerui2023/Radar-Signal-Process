function [iq_data_13beam, raw_iq_data_16ch, servo_angle, frame_headers, success, is_end] = process_stage1_read_data(frame_to_read, config, DBF_coeffs_C)
...
%
% 输出参数:
%   iq_data_13beam   - (complex 3D matrix) DBF处理后的一帧13波束I/Q数据。
%   raw_iq_data_16ch - (complex 3D matrix) 原始的一帧16通道I/Q数据。
...
%  25/07/18   XZR      v1.1     增添原始物理16通道iq数据输出
%% 1. 从config结构体中获取所需参数
...
[iq_data_13beam, raw_iq_data_16ch, servo_angle, frame_headers, frameCompleted, is_global_stream_end] = FrameDataRead_xzr(orgDataFilePath, DBF_coeffs_C, Sig_Config, frame_to_read);
