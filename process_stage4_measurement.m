% process_stage4_measurement.m - 参数测量阶段 (处理流程第四阶段)
%
...
function final_log = process_stage4_measurement(prelim_log, mtd_results, config, frame_headers)
%% 1. 初始化
...
final_detections(1:num_detections) = struct(...
    ...
    'amp_sum', 0, 'amp_diff', 0, 'k_value', 0 ...
);
...
for i = 1:num_detections
    ...
    final_detections(i).servo_deg_raw = current_prelim_detection.servo_raw;
